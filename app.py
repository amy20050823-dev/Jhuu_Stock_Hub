...
建議觀察：1. 是否有新題材在新聞中頻繁出現？ 2. 提防高檔爆量避雷針，留意低檔探底神針。
"""
...
def get_stock_advanced_data(stock_dict):
    ...
    for symbol, name in stock_dict.items():
        try:
            ...
            real_body = real_body if real_body > 0 else 0.001 # 避免除以零
            ...
            if action in ["🚀 超級進場點", "💰 強力加碼", "➕ 加碼金叉", "🔴 試水溫"] and is_lightning_rod:
                action, action_prio = "⚡ 爆量避雷針", 11 # 降級警告
            elif action in ["🛑 賣出停損", "🛌 守季線觀察"] and is_bottom_needle:
                action, action_prio = "🔨 探底神針", 2 # 升級買點
